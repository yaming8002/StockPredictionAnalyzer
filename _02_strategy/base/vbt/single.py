...
其餘（向量化回測、台股費用後處理、tick 進位、summary）由基底處理。
...
class VbtSingleStrategy:
    ...
    def build_signals(self, df: pd.DataFrame):
        """
        回傳 (entries, exits) 兩條 bool Series。
        預設 = (buy_signal, sell_signal) 各自獨立算。
        路徑相依出場（如進場以來最高）請覆寫此函式，做單檔逐根掃描。
        """
    ...
    def exec_price(self, df: pd.DataFrame) -> pd.Series:
        """成交價（預設訊號日收盤；tick 進位在後處理做）。可覆寫成隔日開盤等。"""
        return df["close"]
