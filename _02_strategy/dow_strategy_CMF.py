...
class DowStrategy(StockBacktest):
    ...
    def compute_cmf(self, i, window=20):
        """計算 CMF (Chaikin Money Flow) 指標"""
        if i < window:
            return None

        df = self.data.iloc[i - window : i].copy()

        # 計算 Money Flow Multiplier (MFM)
        ...
        # 計算 CMF
        cmf_value = df['MFV'].rolling(window=window).sum() / df['volume'].rolling(window=window).sum()

        return cmf_value.iloc[-1]  # 返回最新的 CMF 值
    ...
    def buy_signal(self, i):
        ...
        # 計算 CMF
        cmf_value = self.compute_cmf(i - 1, window=20)

        # 如果 CMF < 0.2，表示資金流出，不適合進場
        if cmf_value is None or cmf_value < 0.1:
            return False
        ...
        if not np.isnan(turn_high) and close > turn_high and open_price > turn_high:
            # 設定停損價為最近低點（下修1%防止被掃）
            if (
                cmf_value >= 0.1  # 資金流入強勁
                and volume_ma5 * 1.3 < volume
...
def run_dow_backtest(initial_cash=100000):
    ...
    runner = StrategyRunner(
        ...
        label="dow_zigzag_breakout_CMF0_1_volx1_5_nolengthywick_10m_move_stop_value_close",
        log_folder=folder,
    )
