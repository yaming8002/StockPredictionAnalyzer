...
from _03_deeplearning.unit import DEFAULT_FEATURE_COLUMNS
...
class TurtleDeepStrategy(StockBacktest):
    ...
    def __init__(self, stock_id, start_date, end_date, model_path, initial_cash=100000, split_cash=0, label="backtest", loglevel=logging.INFO):
    ...
    def predict_with_model(self, i, columns=DEFAULT_FEATURE_COLUMNS, normalize=True):
        ...
        return float(y_pred[0][0])  # 回傳機率（可由呼叫方決定是否 > 0.5）
...
def run_turtle_deep_list(start_date="2015-01-01", end_date="2019-12-31", model_path="run_turtle_deep.h5", initial_cash=100000):
    db = get_mongo_client()
    ...
    for stock_id in collections:
        backtest = TurtleDeepStrategy(stock_id=stock_id, model_path=model_path, start_date=start_date, end_date=end_date, initial_cash=initial_cash, label=label)
